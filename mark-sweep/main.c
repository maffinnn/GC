...
{
    /* If already marked, we're done. Check this first to avoid recursing on cycles in the object graph. */
    if (object->marked)
        return;
...
void sweep(VM *vm)
{
    ...
    while (*object)
    {
        ...
        {
            /* This object was reached, so unmarked it (for the next GC) and move on to the next */
            (*object)->marked = 0;
            object = &(*object)->next;
        }
    }
...
void test4()
{
    ...
    b->tail = a;
    gc(vm);
    asset(vm->numObjects == 4, "Should have collected objects.");
    freeVM(vm);
}
